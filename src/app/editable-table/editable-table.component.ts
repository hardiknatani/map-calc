...
export class EditableTableComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    ...
    this.VOForm.valueChanges
      .pipe(debounceTime(200), pairwise())
      .subscribe(([prev, next]) => {
        if (JSON.stringify(prev) != JSON.stringify(next)) {
          this.cdr.detectChanges();
          let value = this.VOForm.getRawValue();
          value.VORows.push({
            property: PROPERTIES.MAPCALC_ID,
            value: this.properties[PROPERTIES.MAPCALC_ID],
          });
          value.VORows.push({
            property: 'selected',
  ...
  ngOnChanges(changes: SimpleChanges): void {
    let properties = JSON.parse(
      JSON.stringify(changes['properties']['currentValue'])
    );
        ...
        control.push(
          this.fb.group({
            property: new FormControl({
              value: ele[0],
              disabled: ele[0] == PROPERTIES.MAPCALC_ID ? true : false,
            }),
